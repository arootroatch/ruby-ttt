...
describe Easy do
  ...
  it 'plays any available move at random' do
    ...
    expect(@board.board).to_not eq([0, 1, 2, 3, 4, 5, 6, 7, 8])
  ...
  it 'does not play unavailable moves' do
    ...
    @player.take_turn(@board)
    expect(@board.board).to_not eq([:x, :x, 2, :o, :o, 5, 6, 7, 8])
    expect(@board).to have_received(:available_moves).twice
    expect(@board).to have_received(:update) { |_arg1, arg2| [2, 5, 6, 7, 8].include? arg2 }
  end
end
